...
class Memory:
    ...
    def __init__(
        ...
        logger.info(f"Memory系统初始化完成，启用记忆类型: {list(self.memory_types.keys())}")
    ...
    def add(
        ...
        Args:
            ...
            metadata: 元数据（如 session_id, tags 等）
        ...
        if memory_type in self.memory_types:
            memo_id = self.memory_types[memory_type].add(memo)
            logger.debug(f"添加记忆到 {memory_type}: {memo_id}")
            return memo_id
        else:
    ...
    def update(
        ...
        for memory_type, memory_instance in self.memory_types.items():
            if memory_instance.has_memory(memo_id):
                return memory_instance.update(memo_id, content, importance, metadata)
        
        logger.warning(f"未找到记忆: {memo_id}")
        return False
    ...
    def remove(self, memo_id: str) -> bool:
        ...
        for memory_type, memory_instance in self.memory_types.items():
            if memory_instance.has_memory(memo_id):
                return memory_instance.remove(memo_id)
    ...
    def __str__(self) -> str:
        ...
        return f"Memory(user={self.user_id}, total={stats['total_memories']})"
